...
RECORDS=$(curl -s -X GET "https://$WHM_SERVER:2087/$WHM_CPSESS/json-api/dumpzone?api.version=1&domain=$RECORD_DOMAIN" \
...
if [[ -n "$RECORD_LINE" ]]; then
    RESPONSE=$(curl -s -X GET "https://$WHM_SERVER:2087/$WHM_CPSESS/json-api/removezonerecord?api.version=1&zone=$RECORD_DOMAIN&line=$RECORD_LINE" \
        -H "Authorization: whm $WHM_USERNAME:$WHM_TOKEN" \
        --insecure)
